...
**绝对单位**, px: (pixels), 即像素单位，如果元素是固定大小的，那么px无疑是最好的选择，常用语margin、padding、shadow  
...
小程序中的 WXSS 对 CSS 进行了扩充以及修改, 除了有一些上面地提到的单位外，还增加了 rpx (responsive pixel)，用作响应式单位，需要特殊说明的是，小程序把屏幕宽度设定为750rpx，永久的把屏幕宽度平分成了750份，假设把组件宽度设定为750rpx，则无论在什么设备上，这个组件的宽度都是跟屏幕一样宽.
...
由于Android的源代码是开源的，所有就有很多厂商用Android系统研发了各种尺寸的手机，也就没有标准的1倍图，2倍图，3倍图，而是通过ldpi, mdpi, hdpi, xhdpi, xxhdpi划分了几个密度等级，越往后用在越高清屏幕的手机上。
...
在Android开发中，常用的单位是dp和sp，dp常用语组件，sp常用于字体. 其他还有一些不常用的单位，比如px, in, mm.  
